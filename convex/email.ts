...
const FROM_ADDRESS = process.env.EMAIL_FROM_ADDRESS ?? "Hisskompetens <onboarding@resend.dev>";
...
export async function sendContactNotification(args: {
  ...
  if (!apiKey || !to) {
    console.error(
      "RESEND_API_KEY or CONTACT_NOTIFICATION_EMAIL not configured — skipping notification",
    );
    return {
      success: false,
...
<body style="background-color:#f6f9fc;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',sans-serif">
  <div style="background-color:#ffffff;margin:0 auto;padding:24px;max-width:600px;border-radius:8px">
    <h1 style="font-size:24px;font-weight:700;color:#1a1a2e;margin:0 0 16px">Ny kontaktförfrågan</h1>
    <hr style="border-color:#e5e7eb;margin:16px 0" />
    <p style="font-size:14px;color:#374151;margin:4px 0"><strong>Namn:</strong> ${escapeHtml(args.name)}</p>
    <p style="font-size:14px;color:#374151;margin:4px 0"><strong>E-post:</strong> ${escapeHtml(args.email)}</p>
    ${args.phone ? `<p style="font-size:14px;color:#374151;margin:4px 0"><strong>Telefon:</strong> ${escapeHtml(args.phone)}</p>` : ""}
    <hr style="border-color:#e5e7eb;margin:16px 0" />
    <h2 style="font-size:18px;font-weight:600;color:#1a1a2e;margin:16px 0 8px">Meddelande</h2>
    <p style="font-size:14px;color:#374151;white-space:pre-wrap">${escapeHtml(args.message)}</p>
    <hr style="border-color:#e5e7eb;margin:16px 0" />
    <p style="font-size:12px;color:#9ca3af;margin:16px 0 0">Detta meddelande skickades automatiskt från kontaktformuläret på hisskompetens.se</p>
  </div>
</body>
</html>`;
  ...
  try {
    const response = await fetch(RESEND_API_URL, {
      ...
      body: JSON.stringify({
        from: FROM_ADDRESS,
        to,
        subject: `Kontaktförfrågan från ${args.name}`,
        html,
      }),
    });
    ...
    console.error("Failed to send contact notification:", message);
