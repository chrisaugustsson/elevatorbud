...
// Helper: build detail data from a parsed elevator row
function toDetailData(data: z.infer<typeof confirmImportSchema>["elevators"][number]) {
  return {
    ...
    emergencyPhonePrice: data.emergency_phone_price,
...
export async function confirm(
  ...
  const existingDetailsMap = new Map<string, string>(); // elevatorId → detailsId
        ...
        try {
          ...
          if (r.data.recommended_modernization_year || r.data.budget_amount) {
            await db.insert(elevatorBudgets).values({
              ...
              budgetAmount: r.data.budget_amount,
  ...
  // Return org mapping so subsequent batches can reference newly created orgs
