...
import { Button } from '@elevatorbud/ui/components/ui/button-1';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@elevatorbud/ui/components/ui/select';
...
import { Badge } from '@elevatorbud/ui/components/ui/badge-2';
import {
  Command,
  CommandEmpty,
  CommandGroup,
...
import { Popover, PopoverContent, PopoverTrigger } from '@elevatorbud/ui/components/ui/popover';
...
function DataGridContainer({
  ...
  return (
    <div data-slot="data-grid" className={cn('grid w-full min-w-0', border && 'border border-border rounded-lg overflow-hidden', className)}>
      {children}
    </div>
  );
}
  ...
  return (
    <th
      ...
      className={cn(
        ...
        props.tableLayout?.columnsResizable && column.getCanResize() && 'truncate select-none',
...
function DataGridPagination(props: DataGridPaginationProps) {
  ...
  const setSize = (size: number) => {
    if (mergedProps.onPageSizeChange) {
      mergedProps.onPageSizeChange(size);
    } else {
      table.setPageSize(size);
    }
  };

  const pageBtnClass = 'inline-flex items-center justify-center size-8 rounded-md text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground disabled:pointer-events-none disabled:opacity-50';
  ...
  return (
    ...
    >
      <div className="hidden sm:flex flex-wrap items-center gap-2.5 order-2 sm:order-1">
        {isLoading ? (
      ...
      <div className="flex justify-between sm:justify-end items-center gap-2.5 w-full sm:w-auto order-1 sm:order-2">
        {isLoading ? (
          mergedProps?.infoSkeleton
        ) : (
          <>
            <div className="hidden sm:block text-sm text-muted-foreground text-nowrap">{paginationInfo}</div>
            ...
            {pageCount > 1 && (
              <div className="flex items-center gap-1">
                <button
                  type="button"
                  className={cn(pageBtnClass, 'text-muted-foreground')}
                ...
                {currentGroupStart > 0 && (
                  <button
                    type="button"
                    className={cn(pageBtnClass, 'hidden sm:inline-flex text-muted-foreground')}
                ...
                {currentGroupEnd < pageCount && (
                  <button
                    type="button"
                    className={cn(pageBtnClass, 'hidden sm:inline-flex text-muted-foreground')}
                    onClick={() => setPage(currentGroupEnd)}
                  >
                    ...
                  </button>
                )}
